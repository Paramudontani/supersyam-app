...
export const SupersyamHome: React.FC = () => {
  ...
  // ปรับ signature ให้รองรับการเรียกทั้งแบบ event และแบบไม่มี event
  const handleLogin = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      // กรณีเกิด exception
      setUser({ email });
      alert('เข้าสู่ระบบ (จำลอง) สำเร็จ (เกิดข้อผิดพลาดเชื่อมต่อ Supabase)!');
      setCurrentView('home');
    }
  };
  ...
  const handleRegister = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      alert('เกิดข้อผิดพลาดขณะสมัครสมาชิก (ระบบออฟไลน์หรือปัญหาเชื่อมต่อ)');
    }
  };

  // ฐานข้อมูลสินค้าบริการท่องเที่ยวไทยแบบครบวงจร (ตัวอย่างข้อมูล)
  const thaiProducts: Record<string, Array<{ id: string; name: string; price: number; type: string; desc: string; image: string }>> = {
    hotels: [
      { id: 'h1', name: 'โรงแรมหรู 5 ดาว ริมแม่น้ำเจ้าพระยา กรุงเทพฯ', price: 4200, type: 'โรงแรม', desc: 'วิวแม่น้ำพร้อมสิ่งอำนวยความสะดวกครบครัน เหมาะสำหรับวันพักผ่อน', image: '🏨' },
      { id: 'h2', name: 'พูลวิลล่าส่วนตัว หาดป่าตอง ภูเก็ต', price: 6500, type: 'พูลวิลล่า', desc: 'วิลล่าส่วนตัว พร้อมสระว่ายน้ำและบริการเชฟส่วนตัว', image: '🏝️' },
      { id: 'h3', name: 'รีสอร์ตกลางขุนเขาและสายหมอก เชียงใหม่', price: 2900, type: 'รีสอร์ต', desc: 'บรรยากาศสงบ เหมาะสำหรับคู่รักและการพักผ่อนยาว', image: '🏞️' },
    ],
    tours: [
      { id: 't1', name: 'ทัวร์ล่องเรือสปีดโบ๊ทเกาะพีพี & อ่าวมาหยา', price: 1500, type: 'ทัวร์', desc: 'ทัวร์วันเดียว เที่ยวเกาะพีพี แวะแหล่งดำน้ำตื้น', image: '⛴️' },
      { id: 't2', name: 'บัตรเข้าชมมหานคร สกายวอล์ค (ชั้น 78)', price: 880, type: 'ตั๋วท่องเที่ยว', desc: 'ชมวิวเมืองกรุงเทพฯ จากมุมสูง', image: '🎟️' },
      { id: 't3', name: 'แพ็กเกจดินเนอร์ล่องเรือหรูแม่น้ำเจ้าพระยา', price: 1200, type: 'ดินเนอร์', desc: 'ดินเนอร์พร้อมวงดนตรีสดและวิวริมแม่น้ำ', image: '🛥️' },
    ],
    cars: [
      { id: 'c1', name: 'รถเช่าขับเอง ท่าอากาศยานสุวรรณภูมิ (Toyota Vios/City)', price: 950, type: 'รถเช่า', desc: 'รถประหยัด เหมาะสำหรับผู้โดยสาร 4 ท่าน', image: '🚗' },
      { id: 'c2', name: 'รถตู้ VIP พร้อมคนขับนำเที่ยวส่วนตัว เชียงใหม่', price: 2500, type: 'รถเช่า', desc: 'รถตู้สำหรับกรุ๊ปทัวร์ พร้อมคนขับและมัคคุเทศก์', image: '🚐' },
    ],
    food: [
      { id: 'f1', name: 'บัตรรับประทานอาหารบุฟเฟต์ ใบหยก 2 (Bangkok Balcony)', price: 850, type: 'ร้านอาหาร', desc: 'บุฟเฟต์นานาชาติ วิวเมือง', image: '🍽️' },
      { id: 'f2', name: 'ดีลสตรีทฟู้ดมิชลินย่านเยาวราช', price: 500, type: 'ร้านอาหาร', desc: 'รวมเมนูสตรีทฟู้ดยอดนิยมจากร้านแนะนำ', image: '🍜' },
    ],
    esim: [
      { id: 'e1', name: 'Thailand 5G eSIM เน็ตไม่จำกัด 10 วัน (Dtac/True)', price: 390, type: 'eSIM', desc: 'สแกน QR Code และใช้งานทันที ครอบคลุม 5G/4G', image: '📱' },
    ],
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-slate-950">
        ...
        <main className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="text-center mb-12 bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 p-8 md:p-12 rounded-3xl shadow-2xl relative overflow-hidden">
            ...
            <p className="text-slate-400 max-w-2xl mx-auto mb-8 text-sm md:text-base">
              รวมโรงแรมหรู พูลวิลล่า ตั๋วท่องเที่ยว รถเช่า อาหารมิชลิน และ eSIM พร้อมดีลพิเศษตลอดปี
            </p>
        ...
        <div className="max-w-md mx-auto my-16 p-8 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl">
          ...
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleLogin(); }}>
            <div>
              ...
              <input type="email" placeholder="name@example.com" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-slate-400 mb-1">รหัสผ่าน</label>
              <input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:outline-none" />
            </div>
      ...
      {currentView === 'dashboard' && (
        <div className="max-w-xl mx-auto my-16 p-8 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl text-center">
          ...
          <button onClick={async () => { await supabase.auth.signOut().catch(() => {}); setUser(null); setCurrentView('home'); }} className="px-6 py-2.5 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-xl font-medium">
            ออกจากระบบ
          </button>
