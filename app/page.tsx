  ...
  return (
    <main className="relative min-h-screen bg-[#070707] text-white selection:bg-amber-400 selection:text-black overflow-hidden font-sans">
      ...
      <nav className="flex items-center justify-between px-8 py-5 border-b border-white/10 backdrop-blur-xl sticky top-0 z-50 bg-[#070707]/70 transition-all duration-300">
        <Link href="/" className="text-2xl font-black tracking-[0.2em] bg-gradient-to-r from-amber-100 via-amber-300 to-amber-500 bg-clip-text text-transparent drop-shadow-[0_2px_10px_rgba(245,158,11,0.2)]">
          SUPERSYAM
        </Link>
        
        {/* เมนูการเชื่อมโยงจริง */}
        <div className="hidden md:flex items-center space-x-10 text-xs font-medium tracking-[0.15em] text-neutral-400 uppercase">
          <Link href="/privileges" className="hover:text-amber-300 transition-colors duration-200">
            เอกสิทธิ์เหนือระดับ
          </Link>
          <Link href="/collections" className="hover:text-amber-300 transition-colors duration-200">
            คอลเลกชัน
          </Link>
        ...
        {/* ปุ่มเข้าสู่ระบบ นำทางไปยังหน้า /login */}
        <div className="flex items-center space-x-4">
          <Link 
            href="/login"
            className="relative inline-flex items-center justify-center p-0.5 overflow-hidden text-xs font-semibold tracking-widest text-black uppercase rounded-full group bg-gradient-to-br from-amber-200 via-amber-400 to-amber-600 hover:shadow-[0_0_25px_rgba(245,158,11,0.35)] transition-all duration-300"
          >
      ...
      <div className="relative z-10">
        ...
        <section className="relative px-8 py-20 md:py-32 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            {/* ฟอร์มค้นหา นำทางไปยังหน้า /search */}
            <form 
              action="/search" 
              method="GET"
              className="p-2 bg-neutral-900/60 border border-white/10 rounded-2xl backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col md:flex-row items-center gap-2 focus-within:border-amber-500/40 transition-colors duration-300"
            >
              <input 
                type="text" 
                name="q"
          ...
          {/* Luxury Card ฝั่งขวาพร้อมปุ่มชำระเงินจริง */}
          <div className="lg:col-span-5 relative">
            <div className="absolute -inset-1 bg-gradient-to-r from-amber-500/20 to-yellow-600/10 rounded-3xl blur-xl opacity-50 transition duration-1000" />
            ...
            <div className="relative w-full aspect-[4/5] rounded-3xl overflow-hidden border border-white/10 bg-gradient-to-b from-neutral-900/90 to-black/90 p-8 flex flex-col justify-between shadow-2xl backdrop-blur-md">
              ...
              <div className="z-10 space-y-6">
                ...
                {/* ปุ่มจอง/ชำระเงิน เรียกใช้ CheckoutButton ที่เชื่อมต่อระบบชำระเงินจริง */}
                <CheckoutButton />
              </div>
            </div>
          </div>
